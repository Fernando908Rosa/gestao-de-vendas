...
public class GestaoVendasTratamentoExcecao extends ResponseEntityExceptionHandler {
	...
	public ResponseEntity<Object> HandleResultDataAcessException(EmptyResultDataAccessException ex, WebRequest request){
	...
	public ResponseEntity<Object> handleRegraNegocioException(RegraNegocioException ex, WebRequest request) {
        String msUsuario = ex.getMessage();
        String msDesenvolvedor = ex.getMessage();
        List<Erro> erros = Arrays.asList(new Erro(msUsuario, msDesenvolvedor));
        return handleExceptionInternal(ex, erros, new HttpHeaders(), HttpStatus.BAD_REQUEST, request);
	}
	...
	private String tratarMensagemDeErroParaUsuario(FieldError fieldError) {
		...
		if (fieldError.getCode().equals(CONSTANT_VALIDATION_MIN)) {
			return fieldError.getDefaultMessage().concat(String.format(" deve ser maior ou iqual a %s",
